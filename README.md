...
**功能用途：**
可伴提供 AI 生成的轻音乐作为背景音乐，旨在为用户营造轻松愉悦的穿搭体验氛围。音乐风格与应用「温柔陪伴」的产品定位一致，帮助用户在记录穿搭、浏览衣橱时获得更沉浸的体验。

**入口位置：**
打开应用 → 点击底部导航栏「设置」→ 在「偏好」分组中点击「背景音乐」→ 进入背景音乐播放页面，可选择曲目、调节音量、播放/暂停。

**后台播放说明：**
本应用使用 iOS 后台音频播放能力（UIBackgroundModes: audio），允许用户在锁定屏幕或切换到其他应用时继续播放背景音乐。这是为了：
- 用户可以在整理衣橱、准备出门时，无需保持屏幕常亮即可继续聆听
- 提供更自然的「陪伴感」，符合应用「衣橱里的温柔时光」的产品理念
- 所有音乐均由 AI 算法生成，我们拥有完整版权

**技术实现：**
- 使用 `just_audio` 插件播放本地音频文件（assets/mp3/）
- iOS 端配置 `AVAudioSession` 为 `.playback` 模式
- Info.plist 中声明 `UIBackgroundModes: audio`
