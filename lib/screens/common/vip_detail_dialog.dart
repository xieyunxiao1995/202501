...
Future<void> showVipDetailDialog(BuildContext context) {
  return showModalBottomSheet(
    context: context,
    isScrollControlled: true,
    backgroundColor: Colors.transparent,
    builder: (context) => const VipDetailDialog(),
  );
}

class VipDetailDialog extends StatefulWidget {
  const VipDetailDialog({super.key});

  @override
  State<VipDetailDialog> createState() => _VipDetailDialogState();
}

class _VipDetailDialogState extends State<VipDetailDialog> {
  // 购买管理器
    ...
    _purchaseManager.initialize(
      callbacks: PurchaseCallbacks(
        onCanceled: () {
          ...
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(content: Text('已取消支付')),
            );
          }
        },
        onError: (message) {
          ...
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(content: Text('支付失败: $message')),
            );
          }
        },
        ...
        onVerificationFailed: (message) {
          ...
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(content: Text('支付验证失败: $message')),
            );
          }
        },
      ),
    ...
    return Transform(
      ...
      child: Container(
        ...
        child: Text(
          ...
          style: TextStyle(
            fontSize: 31.sp,
            color: AppColors.upgradeToVipColor,
